...
flakySuite('Workspaces', () => {
	...
	test('workspace identifiers are stable', function () {
		...
		assert.strictEqual(getWorkspaceIdentifier(URI.parse('zycode-remote:/hello/test')).id, '786de4f224d57691f218dc7f31ee2ee3');

		// single folder identifier (remote)
		assert.strictEqual(getSingleFolderWorkspaceIdentifier(URI.parse('zycode-remote:/hello/test'))?.id, '786de4f224d57691f218dc7f31ee2ee3');
	});
});
