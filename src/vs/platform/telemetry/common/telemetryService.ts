...
export class TelemetryService implements ITelemetryService {
	...
	constructor(
		...
		// static cleanup pattern for: `zycode-file:///DANGEROUS/PATH/resources/app/Useful/Information`
		this._cleanupPatterns = [/(zycode-)?file:\/\/\/.*?\/resources\/app\//gi];
...
function getTelemetryLevelSettingDescription(): string {
	...
	const externalLinksStatement = !product.privacyStatementUrl ?
		localize("telemetry.docsStatement", "Read more about the [data we collect]({0}).", 'https://aka.ms/zycode-telemetry') :
		localize("telemetry.docsAndPrivacyStatement", "Read more about the [data we collect]({0}) and our [privacy statement]({1}).", 'https://aka.ms/zycode-telemetry', product.privacyStatementUrl);
