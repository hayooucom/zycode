...
export class DiskFileSystemProvider extends AbstractDiskFileSystemProvider implements
	...
	async open(resource: URI, opts: IFileOpenOptions): Promise<number> {
			...
			if (isFileOpenForWriteOptions(opts)) {
				if (isWindows) {
					try {
						...
						// (see https://github.com/microsoft/zycode/issues/931) and prevent removing alternate data streams
		...
		if (lock) {
			...
			// https://github.com/microsoft/zycode/issues/142462
			if (previousLock) {
	...
	async close(fd: number): Promise<void> {
		...
		try {
			...
			if (this.writeHandles.delete(fd) && DiskFileSystemProvider.canFlush) {
				try {
					await Promises.fdatasync(fd); // https://github.com/microsoft/zycode/issues/9589
				} catch (error) {
	...
	private normalizePos(fd: number, pos: number): number | null {
		...
		// see https://github.com/microsoft/zycode/issues/73884).
		//
