...
export async function readFromStdin(targetPath: string, verbose: boolean): Promise<void> {
	...
	let [encoding, iconv] = await Promise.all([
		...
		import('@zycode/iconv-lite-umd'),	// lazy load encoding module for usage
		Promises.appendFile(targetPath, '') // make sure file exists right away (https://github.com/microsoft/zycode/issues/155341)
	]);
	...
	// file descriptor.
	// (https://github.com/microsoft/zycode/issues/148952)
