...
export class TabsTitleControl extends TitleControl {
	...
	private updateTabSizing(fromEvent: boolean): void {
		...
		if (options.tabSizing === 'fixed') {
			...
			// For https://github.com/microsoft/zycode/issues/40290 we want to
	...
	private registerTabsContainerListeners(tabsContainer: HTMLElement, tabsScrollbar: ScrollableElement): void {
		...
		// Prevent auto-scrolling (https://github.com/microsoft/zycode/issues/16690)
		...
		this._register(new DragAndDropObserver(tabsContainer, {
			onDragEnter: e => {
				...
				if (e.target !== tabsContainer) {
					this.updateDropFeedback(tabsContainer, false); // fixes https://github.com/microsoft/zycode/issues/52093
					return;
				}
		...
		this._register(addDisposableListener(tabsContainer, EventType.MOUSE_WHEEL, (e: WheelEvent) => {
			...
			// Ignore event if the last one happened too recently (https://github.com/microsoft/zycode/issues/96409)
	...
	private registerTabListeners(tab: HTMLElement, index: number, tabsContainer: HTMLElement, tabsScrollbar: ScrollableElement): IDisposable {
		...
		const handleClickOrTouch = (e: MouseEvent | GestureEvent, preserveFocus: boolean): void => {
			...
			if (e instanceof MouseEvent && (e.button !== 0 /* middle/right mouse button */ || (isMacintosh && e.ctrlKey /* macOS context menu */))) {
				if (e.button === 1) {
					e.preventDefault(); // required to prevent auto-scrolling (https://github.com/microsoft/zycode/issues/16690)
				}
		...
		disposables.add(addDisposableListener(tab, EventType.AUXCLICK, e => {
			if (e.button === 1 /* Middle Button*/) {
				EventHelper.stop(e, true /* for https://github.com/microsoft/zycode/issues/56715 */);
		...
		}, true /* use capture to fix https://github.com/microsoft/zycode/issues/19145 */));
		...
		disposables.add(addDisposableListener(tab, EventType.DRAG_START, e => {
			...
			// Fixes https://github.com/microsoft/zycode/issues/18733
	...
	private isSupportedDropTransfer(e: DragEvent): boolean {
		...
		if (e.dataTransfer && e.dataTransfer.types.length > 0) {
			return true; // optimistically allow external data (// see https://github.com/microsoft/zycode/issues/25789)
		}
	...
	private redrawTabLabel(editor: EditorInput, index: number, tabContainer: HTMLElement, tabLabelWidget: IResourceLabel, tabLabel: IEditorInputLabel): void {
		...
		if (tabLabel.ariaLabel) {
			...
			// More details https://github.com/microsoft/zycode/issues/95378
		...
		if (this.accessor.partOptions.wrapTabs) {
			...
			const lastTabFitsWrapped = () => {
				...
				if (lastTabOverlapWithToolbarWidth > 1) {
					...
					// https://github.com/microsoft/zycode/issues/116385
					return false;
				}
			...
			// flickering issue https://github.com/microsoft/zycode/issues/115050)
	...
	private doLayoutTabsNonWrapping(activeTab: HTMLElement, activeIndex: number, options?: ITabsTitleControlLayoutOptions): void {
		...
		if (
			...
			(!dimensionsChanged && !options?.forceRevealActiveTab) 	// dimensions did not change and we have low layout priority (https://github.com/microsoft/zycode/issues/133631)
		) {
...
registerThemingParticipant((theme, collector) => {
	...
	// - if we have a contrast border (which draws an outline - https://github.com/microsoft/zycode/issues/109117)
	// - on Safari (https://github.com/microsoft/zycode/issues/108996)
