...
suite('terminalUri', () => {
	suite('getTerminalResourcesFromDragEvent', () => {
		...
		test('should return terminal resource when event contains one', () => {
			deepStrictEqual(
				getTerminalResourcesFromDragEvent(fakeDragEvent('["zycode-terminal:/1626874386474/3"]'))?.map(e => e.toString()),
				['zycode-terminal:/1626874386474/3']
			);
		});
		test('should return multiple terminal resources when event contains multiple', () => {
			deepStrictEqual(
				getTerminalResourcesFromDragEvent(fakeDragEvent('["zycode-terminal:/foo/1","zycode-terminal:/bar/2"]'))?.map(e => e.toString()),
				['zycode-terminal:/foo/1', 'zycode-terminal:/bar/2']
			);
		});
	});
