	...
	private async registerListeners(): Promise<void> {
		...
		this._register(this.workspaceTrustRequestService.onDidInitiateOpenFilesTrustRequest(async () => {
			...
			const markdownDetails = [
				...
				localize('openLooseFileLearnMore', "If you don't want to open untrusted files, we recommend to open them in Restricted Mode in a new window as the files may be malicious. See [our docs](https://aka.ms/zycode-workspace-trust) to learn more.")
			];
			...
			const { result } = await this.dialogService.prompt({
				...
				custom: {
					...
					markdownDetails: [
						...
						{ markdown: new MarkdownString(localize('immediateTrustRequestLearnMore', "If you don't trust the authors of these files, we do not recommend continuing as the files may be malicious. See [our docs](https://aka.ms/zycode-workspace-trust) to learn more.")) }
					]
				},
		...
		this._register(this.workspaceTrustRequestService.onDidInitiateWorkspaceTrustRequestOnStartup(async () => {
			...
			this.doShowModal(
				...
				[
					...
					learnMoreString ?? localize('startupTrustRequestLearnMore', "If you don't trust the authors of these files, we recommend to continue in restricted mode as the files may be malicious. See [our docs](https://aka.ms/zycode-workspace-trust) to learn more."),
					!isEmptyWindow ?
		...
		const actions =
			[
				...
				},
				{
					...
					href: 'https://aka.ms/zycode-workspace-trust'
				}
			];
