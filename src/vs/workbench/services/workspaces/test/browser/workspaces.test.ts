...
suite('Workspaces', () => {
	test('workspace identifiers are stable', function () {
		...
		assert.strictEqual(getWorkspaceIdentifier(URI.parse('zycode-remote:/hello/test')).id, '474434e4');

		// single folder identifier
		assert.strictEqual(getSingleFolderWorkspaceIdentifier(URI.parse('zycode-remote:/hello/test'))?.id, '474434e4');
	});
});
