...
async function rimrafMove(path: string, moveToPath = randomPath(tmpdir())): Promise<void> {
	try {
		try {
			...
			// via unlink.
			// https://github.com/microsoft/zycode/issues/139908
...
function doWriteFileAndFlush(path: string, data: string | Buffer | Uint8Array, options: IEnsuredWriteFileOptions, callback: (error: Error | null) => void): void {
	...
	fs.open(path, options.flag, options.mode, (openError, fd) => {
		...
		fs.writeFile(fd, data, writeError => {
			...
			// https://github.com/microsoft/zycode/issues/9589
...
export function writeFileSync(path: string, data: string | Buffer, options?: IWriteFileOptions): void {
	...
	try {
		...
		// Flush contents (not metadata) of the file to disk
		try {
			fs.fdatasyncSync(fd); // https://github.com/microsoft/zycode/issues/9589
		} catch (syncError) {
...
async function doCopy(source: string, target: string, payload: ICopyPayload): Promise<void> {
	...
	if (symbolicLink) {
		...
		if (symbolicLink.dangling) {
			return; // skip dangling symbolic links from here on (https://github.com/microsoft/zycode/issues/111621)
		}
	}
 ...
 * one (https://github.com/microsoft/zycode/issues/118562)
 *
 ...
 * (https://github.com/microsoft/zycode/issues/124176)
 */
