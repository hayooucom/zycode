...
declare module 'zycode' {
	...
	*/
	...
	export class RelativePattern {
		 ...
		 * Example:
		 * ```ts
		 * const folder = zycode.workspace.workspaceFolders?.[0];
		 * if (folder) {
		 *
		 *   // Match any TypeScript file in the root of this workspace folder
		 *   const pattern1 = new zycode.RelativePattern(folder, '*.ts');
		 *
		 *   // Match any TypeScript file in `someFolder` inside this workspace folder
		 *   const pattern2 = new zycode.RelativePattern(folder, 'someFolder/*.ts');
		 * }
		 * ```
		 *
	...
	export class MarkdownString {
		 ...
		 * and attributes to be rendered. See https://github.com/microsoft/zycode/blob/6d2920473c6f13759c978dd89104c4270a83422d/src/vs/base/browser/markdownRenderer.ts#L296
		 ...
		 *
		 * ```ts
		 * const md = new zycode.MarkdownString(`[link](./file.js)`);
		 * md.baseUri = zycode.Uri.file('/path/to/dir/');
		 ...
		 * ```ts
		 * const md = new zycode.MarkdownString(`[link](./file.js)`);
		 * md.baseUri = zycode.Uri.file('/path/to/otherFile.js');
	 ...
	 * const config = workspace.getConfiguration('launch', zycode.workspace.workspaceFolders[0].uri);
	 *
	 * // retrieve values
	...
	export interface Terminal {
		 ...
		 *   	zycode.window.showInformationMessage(`Exit code: ${t.exitStatus.code}`);
		 *   }
		 * });
		 * ```
		 */
	...
	export interface ExtensionContext {
		 ...
		 * {@linkcode ExtensionContext.extensionUri extensionUri}, e.g. `zycode.Uri.joinPath(context.extensionUri, relativePath);`
		 *
	 ...
	 * a file or folder doesn't exist, use them like so: `throw zycode.FileSystemError.FileNotFound(someUri);`
	 */
	...
	export interface Webview {
		 ...
		 * ```html
		 * <script>
		 *     const zycode = acquireVsCodeApi(); // acquireVsCodeApi can only be invoked once
		 *     zycode.postMessage({ message: 'hello!' });
		 * </script>
		 * ```
		 *
		 ...
		 * setting a [content security policy](https://aka.ms/zycode-api-webview-csp).
		 */
		html: string;
		 ...
		 *
		 *   For older versions of zycode, if an `ArrayBuffer` is included in `message`,
		 ...
		 *
		 *   However if your extension targets zycode 1.57+ in the `engines` field of its
		 ...
		 *
		 * ```ts
		 * webview.html = `<img src="${webview.asWebviewUri(zycode.Uri.file('/Users/codey/workspace/cat.gif'))}">`
		 * ```
		 */
	...
	/**
	 * Restore webview panels that have been persisted when zycode shuts down.
	 *
	 ...
	 *
	 * // Get existing state
	 * const oldState = zycode.getState() || { value: 0 };
	 *
	 * // Update state
	...
	export namespace env {
		 ...
		 * * VSCode itself (`zycode:` from `zycode.env.uriScheme`)
		 *
		 ...
		 * ```typescript
		 * zycode.window.registerUriHandler({
		 *   handleUri(uri: zycode.Uri): zycode.ProviderResult<void> {
		 ...
		 * });
		 *
		 * const callableUri = await zycode.env.asExternalUri(zycode.Uri.parse(zycode.env.uriScheme + '://my.extension/did-authenticate'));
		 * await zycode.env.openExternal(callableUri);
		 * ```
		 *
	...
	export namespace window {
		 ...
		 * the following rules:
		 *
		 * - The uri-scheme must be `zycode.env.uriScheme`;
	...
	export class TreeItem {
		 ...
		 *
		 * Please use `zycode.open` or `zycode.diff` as command IDs when the tree item is opening
	...
	*/
	...
	interface Pseudoterminal {
		 ...
		 * ```typescript
		 * const writeEmitter = new zycode.EventEmitter<string>();
		 * const pty: zycode.Pseudoterminal = {
		 *   onDidWrite: writeEmitter.event,
		 *   open: () => writeEmitter.fire('\x1b[31mHello world\x1b[0m'),
		 *   close: () => {}
		 * };
		 * zycode.window.createTerminal({ name: 'My terminal', pty });
		 * ```
		 *
		 ...
		 * ```typescript
		 * const dimensionsEmitter = new zycode.EventEmitter<zycode.TerminalDimensions>();
		 * const pty: zycode.Pseudoterminal = {
		 *   onDidWrite: writeEmitter.event,
		 ...
		 * ```typescript
		 * const writeEmitter = new zycode.EventEmitter<string>();
		 * const closeEmitter = new zycode.EventEmitter<void>();
		 * const pty: zycode.Pseudoterminal = {
		 *   onDidWrite: writeEmitter.event,
		 ...
		 *   handleInput: data => {
		 *     if (data !== 'y') {
		 *       zycode.window.showInformationMessage('Something went wrong');
		 *     }
		 *     closeEmitter.fire();
		 *   }
		 * };
		 * const terminal = zycode.window.createTerminal({ name: 'Exit example', pty });
		 * terminal.show(true);
		 * ```
		 */
		 ...
		 * ```typescript
		 * const writeEmitter = new zycode.EventEmitter<string>();
		 * const changeNameEmitter = new zycode.EventEmitter<string>();
		 * const pty: zycode.Pseudoterminal = {
		 *   onDidWrite: writeEmitter.event,
		 ...
		 * };
		 * zycode.window.createTerminal({ name: 'Local echo', pty });
		 * ```
		 */
	...
	export namespace workspace {
		 ...
		 * files, e.g. `zycode.workspace.fs.readDirectory(someUri)` allows to retrieve all entries
		 * of a directory or `zycode.workspace.fs.stat(anotherUri)` returns the meta data for a
		 * file.
		 */
		 ...
		 *
		 * The location can e.g. be used with the `zycode.openFolder` command to
		 * open the workspace again after it has been closed.
		 *
		 * **Example:**
		 * ```typescript
		 * zycode.commands.executeCommand('zycode.openFolder', uriOfWorkspace);
		 * ```
		 *
		 * Refer to https://code.visualstudio.com/docs/editor/workspaces for more information on
		 * the concept of workspaces.
		 ...
		 * by an optional set of `workspaceFoldersToAdd` on the `zycode.workspace.workspaceFolders` array. This "splice"
		 ...
		 *
		 * ```ts
		 * const watcher = zycode.workspace.createFileSystemWatcher(new zycode.RelativePattern(<folder>, <pattern>));
		 *
		 ...
		 *
		 * ```ts
		 * zycode.workspace.createFileSystemWatcher(new zycode.RelativePattern(zycode.workspace.workspaceFolders[0], '**​/*.js'));
		 * ```
		 *
		 * If you want to monitor file events across all opened workspace folders:
		 *
		 * ```ts
		 * zycode.workspace.createFileSystemWatcher('**​/*.js');
		 * ```
		 *
		 ...
		 *
		 * ```ts
		 * zycode.workspace.createFileSystemWatcher(new zycode.RelativePattern(zycode.Uri.file(<path to folder outside workspace>), '*.js'));
		 * ```
		 *
		 ...
		 *
		 * ```ts
		 * zycode.workspace.createFileSystemWatcher(new zycode.RelativePattern(zycode.window.activeTextEditor.document.uri, '*'));
		 * ```
		 *
	...
	export namespace languages {
		 ...
		 * doc.uri; // `zycode-notebook-cell:///my/notebook.ipynb#gl65s2pmha`;
	...
	export class NotebookCellOutput {
		 ...
		 * 	zycode.NotebookCellOutputItem.text('Hello', 'text/plain'),
		 * 	zycode.NotebookCellOutputItem.text('<i>Hello</i>', 'text/html'),
		 * 	zycode.NotebookCellOutputItem.text('_Hello_', 'text/markdown'),
		 * 	zycode.NotebookCellOutputItem.text('Hey', 'text/plain'), // INVALID: repeated type, editor will pick just one
		 * ])
		 * ```
		 */
	...
	export namespace debug {
		 ...
		 * The named configurations are looked up in '.zycode/launch.json' found in the given folder.
	...
	*/
	 ...
	 * [zycode-l10n repo](https://github.com/microsoft/zycode-l10n).
	 *
	 ...
	 *
	 * ```js
	 * const sender: zycode.TelemetrySender = {...};
	 * const logger = zycode.env.createTelemetryLogger(sender);
	 *
