...
namespace IntellisenseState {
	...
	export class Pending {
		...
		public readonly cancellation = new zycode.CancellationTokenSource();
...
export class IntellisenseStatus extends Disposable {
	...
	private _statusItem?: zycode.LanguageStatusItem;
	...
	constructor(
		...
		commandManager.register({
			...
			execute: async (root: zycode.Uri, projectType: ProjectType) => {
	...
	private updateState(newState: IntellisenseState.State): void {
		...
		switch (this._state.type) {
			...
			case IntellisenseState.Type.Pending: {
				...
				statusItem.severity = zycode.LanguageStatusSeverity.Information;
				statusItem.text = zycode.l10n.t("Loading IntelliSense status");
			...
			case IntellisenseState.Type.Resolved: {
				const noConfigFileText = this._state.projectType === ProjectType.TypeScript
					? zycode.l10n.t("No tsconfig")
					: zycode.l10n.t("No jsconfig");

				const rootPath = this._client.getWorkspaceRootForResource(this._state.resource);
				if (!rootPath) {
					if (this._statusItem) {
						this._statusItem.text = noConfigFileText;
						this._statusItem.detail = !zycode.workspace.workspaceFolders
							? zycode.l10n.t("No opened folders")
							: zycode.l10n.t("File is not part opened folders");
				...
				if (isImplicitProjectConfigFile(this._state.configFile)) {
					...
					statusItem.command = {
						...
						title: this._state.projectType === ProjectType.TypeScript
							? zycode.l10n.t("Configure tsconfig")
							: zycode.l10n.t("Configure jsconfig"),
						arguments: [rootPath],
					};
				} else {
					statusItem.text = zycode.workspace.asRelativePath(this._state.configFile);
					statusItem.detail = undefined;
					statusItem.command = {
						command: this.openOpenConfigCommandId,
						title: zycode.l10n.t("Open config file"),
						arguments: [rootPath],
					};
				}
				break;
			...
			case IntellisenseState.Type.SyntaxOnly: {
				const statusItem = this.ensureStatusItem();
				statusItem.severity = zycode.LanguageStatusSeverity.Warning;
				statusItem.text = zycode.l10n.t("Partial Mode");
				statusItem.detail = zycode.l10n.t("Project Wide IntelliSense not available");
				statusItem.busy = false;
				statusItem.command = {
					title: zycode.l10n.t("Learn More"),
					command: 'zycode.open',
					arguments: [
						zycode.Uri.parse('https://aka.ms/zycode/jsts/partial-mode'),
					]
				};
				break;
			}
	...
	private ensureStatusItem(): zycode.LanguageStatusItem {
		if (!this._statusItem) {
			this._statusItem = zycode.languages.createLanguageStatusItem('typescript.projectStatus', jsTsLanguageModes);
			this._statusItem.name = zycode.l10n.t("JS/TS IntelliSense Status");
		}
