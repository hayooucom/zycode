...
export default class MergeDecorator implements zycode.Disposable {

	private decorations: { [key: string]: zycode.TextEditorDecorationType } = {};
	...
	private updating = new Map<zycode.TextEditor, boolean>();

	constructor(private context: zycode.ExtensionContext, trackerService: interfaces.IDocumentMergeConflictTrackerService) {
	...
	begin(config: interfaces.IExtensionConfiguration) {
		...
		zycode.window.visibleTextEditors.forEach(e => this.applyDecorations(e));

		zycode.workspace.onDidOpenTextDocument(event => {
			this.applyDecorationsFromEvent(event);
		}, null, this.context.subscriptions);

		zycode.workspace.onDidChangeTextDocument(event => {
	...
	configurationUpdated(config: interfaces.IExtensionConfiguration) {
		...
		zycode.window.visibleTextEditors.forEach(e => {
	...
	private registerDecorationTypes(config: interfaces.IExtensionConfiguration) {
		...
		// Create decorators
		if (config.enableDecorations || config.enableEditorOverview) {
			this.decorations['current.content'] = zycode.window.createTextEditorDecorationType(
				this.generateBlockRenderOptions('merge.currentContentBackground', 'editorOverviewRuler.currentContentForeground', config)
			);

			this.decorations['incoming.content'] = zycode.window.createTextEditorDecorationType(
				this.generateBlockRenderOptions('merge.incomingContentBackground', 'editorOverviewRuler.incomingContentForeground', config)
			);

			this.decorations['commonAncestors.content'] = zycode.window.createTextEditorDecorationType(
		...
		if (config.enableDecorations) {
			this.decorations['current.header'] = zycode.window.createTextEditorDecorationType({
				isWholeLine: this.decorationUsesWholeLine,
				backgroundColor: new zycode.ThemeColor('merge.currentHeaderBackground'),
				color: new zycode.ThemeColor('editor.foreground'),
				outlineStyle: 'solid',
				outlineWidth: '1pt',
				outlineColor: new zycode.ThemeColor('merge.border'),
				after: {
					contentText: ' ' + zycode.l10n.t("(Current Change)"),
					color: new zycode.ThemeColor('descriptionForeground')
				}
			});

			this.decorations['commonAncestors.header'] = zycode.window.createTextEditorDecorationType({
				isWholeLine: this.decorationUsesWholeLine,
				backgroundColor: new zycode.ThemeColor('merge.commonHeaderBackground'),
				color: new zycode.ThemeColor('editor.foreground'),
				outlineStyle: 'solid',
				outlineWidth: '1pt',
				outlineColor: new zycode.ThemeColor('merge.border')
			...
			this.decorations['splitter'] = zycode.window.createTextEditorDecorationType({
				color: new zycode.ThemeColor('editor.foreground'),
				outlineStyle: 'solid',
				outlineWidth: '1pt',
				outlineColor: new zycode.ThemeColor('merge.border'),
			...
			this.decorations['incoming.header'] = zycode.window.createTextEditorDecorationType({
				backgroundColor: new zycode.ThemeColor('merge.incomingHeaderBackground'),
				color: new zycode.ThemeColor('editor.foreground'),
				outlineStyle: 'solid',
				outlineWidth: '1pt',
				outlineColor: new zycode.ThemeColor('merge.border'),
				isWholeLine: this.decorationUsesWholeLine,
				after: {
					contentText: ' ' + zycode.l10n.t("(Incoming Change)"),
					color: new zycode.ThemeColor('descriptionForeground')
				}
			});
		}
	...
	private generateBlockRenderOptions(backgroundColor: string, overviewRulerColor: string, config: interfaces.IExtensionConfiguration): zycode.DecorationRenderOptions {

		const renderOptions: zycode.DecorationRenderOptions = {};

		if (config.enableDecorations) {
			renderOptions.backgroundColor = new zycode.ThemeColor(backgroundColor);
			renderOptions.isWholeLine = this.decorationUsesWholeLine;
		}

		if (config.enableEditorOverview) {
			renderOptions.overviewRulerColor = new zycode.ThemeColor(overviewRulerColor);
			renderOptions.overviewRulerLane = zycode.OverviewRulerLane.Full;
		}

		return renderOptions;
	}

	private applyDecorationsFromEvent(eventDocument: zycode.TextDocument) {
	...
	private async applyDecorations(editor: zycode.TextEditor) {
		...
		try {
			...
			if (zycode.window.visibleTextEditors.indexOf(editor) === -1) {
				return;
			}
			...
			const matchDecorations: { [key: string]: zycode.Range[] } = {};

			const pushDecoration = (key: string, d: zycode.Range) => {
	...
	private removeDecorations(editor: zycode.TextEditor) {
