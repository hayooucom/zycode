...
zycode.workspace.registerFileSystemProvider(testFs.scheme, testFs, { isCaseSensitive: testFs.isCaseSensitive });

export async function createRandomFile(contents = '', dir: zycode.Uri | undefined = undefined, ext = ''): Promise<zycode.Uri> {
	let fakeFile: zycode.Uri;
	if (dir) {
		assert.strictEqual(dir.scheme, testFs.scheme);
		fakeFile = dir.with({ path: dir.path + '/' + rndName() + ext });
	} else {
		fakeFile = zycode.Uri.parse(`${testFs.scheme}:/${rndName() + ext}`);
	}
	testFs.writeFile(fakeFile, Buffer.from(contents), { create: true, overwrite: true });
	return fakeFile;
}

export async function deleteFile(file: zycode.Uri): Promise<boolean> {
	try {
		testFs.delete(file);
		return true;
	} catch {
...
export function saveAllEditors(): Thenable<any> {
	return zycode.commands.executeCommand('workbench.action.files.saveAll');
}

export async function revertAllDirty(): Promise<void> {
	return zycode.commands.executeCommand('_workbench.revertAllDirty');
}

export function disposeAll(disposables: zycode.Disposable[]) {
	zycode.Disposable.from(...disposables).dispose();
}
...
function withLogLevel(level: string, runnable: () => Promise<any>): () => Promise<void> {
	return async (): Promise<void> => {
		const logLevel = await zycode.commands.executeCommand('_extensionTests.getLogLevel');
		await zycode.commands.executeCommand('_extensionTests.setLogLevel', level);

		try {
			await runnable();
		} finally {
			await zycode.commands.executeCommand('_extensionTests.setLogLevel', logLevel);
		}
	};
}
...
export function assertNoRpc() {
	assertNoRpcFromEntry([zycode, 'zycode']);
}
...
export async function asPromise<T>(event: zycode.Event<T>, timeout = zycode.env.uiKind === zycode.UIKind.Desktop ? 5000 : 15000): Promise<T> {
