...
import { commands, env, Position, Range, Selection, SnippetString, TextDocument, TextEditor, TextEditorCursorStyle, TextEditorLineNumbersStyle, Uri, window, workspace } from 'zycode';
import { assertNoRpc, closeAllEditors, createRandomFile, deleteFile } from '../utils';

suite('zycode API - editors', () => {
	...
	test('TextEditor.edit can control undo/redo stack 1', () => {
		return withRandomFileEditor('Hello world!', async (editor, doc) => {
			...
			if (doc.getText() === 'hello world!') {
				// see https://github.com/microsoft/zycode/issues/109131
