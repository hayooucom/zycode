...
suite('zycode - automatic language detection', () => {
	...
	// TODO@TylerLeonhardt https://github.com/microsoft/zycode/issues/135157
	test.skip('test automatic language detection works', async () => {
		const receivedEvent = asPromise(zycode.workspace.onDidOpenTextDocument, 5000);
		const doc = await zycode.workspace.openTextDocument();
		const editor = await zycode.window.showTextDocument(doc);
		await receivedEvent;

		assert.strictEqual(editor.document.languageId, 'plaintext');

		const settingResult = zycode.workspace.getConfiguration().get<boolean>('workbench.editor.languageDetection');
		assert.ok(settingResult);

		const result = await editor.edit(editBuilder => {
			editBuilder.insert(new zycode.Position(0, 0), `{
	...
	"include": [
		...
		let newDoc;
		do {
			newDoc = await asPromise(zycode.workspace.onDidOpenTextDocument, 5000);
