...
suite('Notebook Document', function () {
	...
	const simpleContentProvider = new class implements zycode.NotebookSerializer {
		deserializeNotebook(_data: Uint8Array): zycode.NotebookData | Thenable<zycode.NotebookData> {
			return new zycode.NotebookData(
				[new zycode.NotebookCellData(zycode.NotebookCellKind.Code, '// SIMPLE', 'javascript')],
			);
		}
		serializeNotebook(_data: zycode.NotebookData): Uint8Array | Thenable<Uint8Array> {
	...
	suiteSetup(function () {
		disposables.push(zycode.workspace.registerNotebookSerializer('notebook.nbdtest', simpleContentProvider));
	});

	test('cannot open unknown types', async function () {
		try {
			await zycode.workspace.openNotebookDocument(zycode.Uri.parse('some:///thing.notTypeKnown'));
			assert.ok(false);
		} catch {
			assert.ok(true);
		}
	...
	test('notebook open/close, notebook ready when cell-document open event is fired', async function () {
		...
		const p = new Promise<void>((resolve, reject) => {
			const sub = zycode.workspace.onDidOpenTextDocument(doc => {
				if (doc.uri.scheme !== 'zycode-notebook-cell') {
					// ignore other open events
					return;
				}
				const notebook = zycode.workspace.notebookDocuments.find(notebook => {
		...
		await zycode.workspace.openNotebookDocument(uri);
		await p;
	...
	test('notebook open/close, all cell-documents are ready', async function () {
		...
		const p = utils.asPromise(zycode.workspace.onDidOpenNotebookDocument).then(notebook => {
			for (let i = 0; i < notebook.cellCount; i++) {
				const cell = notebook.cellAt(i);

				const doc = zycode.workspace.textDocuments.find(doc => doc.uri.toString() === cell.document.uri.toString());
				assert.ok(doc);
	...
	test('open untitled notebook', async function () {
		const nb = await zycode.workspace.openNotebookDocument('notebook.nbdtest');
	...
	test('open untitled with data', async function () {
		const nb = await zycode.workspace.openNotebookDocument(
			'notebook.nbdtest',
			new zycode.NotebookData([
				new zycode.NotebookCellData(zycode.NotebookCellKind.Code, 'console.log()', 'javascript'),
				new zycode.NotebookCellData(zycode.NotebookCellKind.Markup, 'Hey', 'markdown'),
			])
		);
		assert.strictEqual(nb.isUntitled, true);
		assert.strictEqual(nb.isClosed, false);
		...
		assert.strictEqual(nb.cellAt(0).kind, zycode.NotebookCellKind.Code);
		assert.strictEqual(nb.cellAt(1).kind, zycode.NotebookCellKind.Markup);
	});
	...
	test('workspace edit API (replaceCells)', async function () {
		...
		// inserting two new cells
		{
			...
			edit.set(document.uri, [zycode.NotebookEdit.replaceCells(new zycode.NotebookRange(0, 0), [{
				kind: zycode.NotebookCellKind.Markup,
				languageId: 'markdown',
				metadata: undefined,
				outputs: [],
				value: 'new_markdown'
		...
		// deleting cell 1 and 3
		{
			...
			edit.set(document.uri, [
				zycode.NotebookEdit.replaceCells(new zycode.NotebookRange(0, 1), []),
				zycode.NotebookEdit.replaceCells(new zycode.NotebookRange(2, 3), [])
			]);
			const success = await zycode.workspace.applyEdit(edit);
			assert.strictEqual(success, true);
		}
		...
		{
			const edit = new zycode.WorkspaceEdit();
			edit.set(document.uri, [zycode.NotebookEdit.replaceCells(new zycode.NotebookRange(0, 1), [{
				kind: zycode.NotebookCellKind.Markup,
				languageId: 'markdown',
				metadata: undefined,
				outputs: [],
		...
		{
			const edit = new zycode.WorkspaceEdit();
			edit.set(document.uri, [zycode.NotebookEdit.replaceCells(new zycode.NotebookRange(0, document.cellCount), [])]);
			const success = await zycode.workspace.applyEdit(edit);
			assert.strictEqual(success, true);
		}
	...
	test('workspace edit API (replaceCells, event)', async function () {
		...
		const event = utils.asPromise<zycode.NotebookDocumentChangeEvent>(zycode.workspace.onDidChangeNotebookDocument);
	...
	test('workspace edit API (replaceMetadata)', async function () {
		...
		const edit = new zycode.WorkspaceEdit();
		edit.set(document.uri, [zycode.NotebookEdit.updateCellMetadata(0, { inputCollapsed: true })]);
		const success = await zycode.workspace.applyEdit(edit);
		assert.strictEqual(success, true);
	...
	test('workspace edit API (notebookMetadata)', async function () {
		...
		const edit = new zycode.WorkspaceEdit();
		const metdataEdit = zycode.NotebookEdit.updateNotebookMetadata({ ...document.metadata, custom: { ...(document.metadata.custom || {}), extraNotebookMetadata: true } });
	...
	test('setTextDocumentLanguage for notebook cells', async function () {
		...
		const pclose = utils.asPromise(zycode.workspace.onDidCloseTextDocument);
		const popen = utils.asPromise(zycode.workspace.onDidOpenTextDocument);

		await zycode.languages.setTextDocumentLanguage(first.document, 'css');
	...
	test('setTextDocumentLanguage when notebook editor is not open', async function () {
		...
		let cellDoc = await zycode.workspace.openTextDocument(firstCelUri);
		cellDoc = await zycode.languages.setTextDocumentLanguage(cellDoc, 'css');
	...
	test.skip('onDidOpenNotebookDocument - emit event only once when opened in two editors', async function () { // TODO@rebornix https://github.com/microsoft/zycode/issues/157222
		const uri = await utils.createRandomFile(undefined, undefined, '.nbdtest');
		let counter = 0;
		testDisposables.push(zycode.workspace.onDidOpenNotebookDocument(nb => {
		...
		await zycode.window.showNotebookDocument(notebook, { viewColumn: zycode.ViewColumn.Active });
		assert.strictEqual(counter, 1);
		assert.strictEqual(zycode.window.visibleNotebookEditors.length, 1);

		await zycode.window.showNotebookDocument(notebook, { viewColumn: zycode.ViewColumn.Beside });
		assert.strictEqual(counter, 1);
