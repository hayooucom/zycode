...
suite('zycode API - workspace events', () => {
	...
	test('onWillCreate/onDidCreate', withLogDisabled(async function () {
		...
		let onWillCreate: zycode.FileWillCreateEvent | undefined;
		let onDidCreate: zycode.FileCreateEvent | undefined;

		disposables.push(zycode.workspace.onWillCreateFiles(e => onWillCreate = e));
		disposables.push(zycode.workspace.onDidCreateFiles(e => onDidCreate = e));
	...
	test('onWillCreate/onDidCreate, make changes, edit another file', withLogDisabled(async function () {
		...
		const base = await createRandomFile();
		const baseDoc = await zycode.workspace.openTextDocument(base);

		const newUri = base.with({ path: base.path + '-foo' });

		disposables.push(zycode.workspace.onWillCreateFiles(e => {
			const ws = new zycode.WorkspaceEdit();
			ws.insert(base, new zycode.Position(0, 0), 'HALLO_NEW');
	...
	test('onWillCreate/onDidCreate, make changes, edit new file fails', withLogDisabled(async function () {
		...
		disposables.push(zycode.workspace.onWillCreateFiles(e => {
			const ws = new zycode.WorkspaceEdit();
			ws.insert(e.files[0], new zycode.Position(0, 0), 'nope');
			e.waitUntil(Promise.resolve(ws));
		}));
		...
		assert.strictEqual((await zycode.workspace.fs.readFile(newUri)).toString(), '');
		assert.strictEqual((await zycode.workspace.openTextDocument(newUri)).getText(), '');
	}));
	...
	test('onWillDelete/onDidDelete', withLogDisabled(async function () {
		...
		let onWilldelete: zycode.FileWillDeleteEvent | undefined;
		let onDiddelete: zycode.FileDeleteEvent | undefined;

		disposables.push(zycode.workspace.onWillDeleteFiles(e => onWilldelete = e));
		disposables.push(zycode.workspace.onDidDeleteFiles(e => onDiddelete = e));
	...
	test('onWillDelete/onDidDelete, make changes', withLogDisabled(async function () {
		...
		disposables.push(zycode.workspace.onWillDeleteFiles(e => {

			const edit = new zycode.WorkspaceEdit();
			edit.createFile(newUri);
			edit.insert(newUri, new zycode.Position(0, 0), 'hahah');
	...
	test('onWillRename/onDidRename', withLogDisabled(async function () {
		...
		let onWillRename: zycode.FileWillRenameEvent | undefined;
		let onDidRename: zycode.FileRenameEvent | undefined;

		disposables.push(zycode.workspace.onWillRenameFiles(e => onWillRename = e));
		disposables.push(zycode.workspace.onDidRenameFiles(e => onDidRename = e));
	...
	async function testOnWillRename(withDirtyFile: boolean): Promise<void> {
		...
		if (withDirtyFile) {
			const edit = new zycode.WorkspaceEdit();
			edit.insert(oldUri, new zycode.Position(0, 0), 'BAR');

			const success = await zycode.workspace.applyEdit(edit);
			assert.ok(success);

			const oldDocument = await zycode.workspace.openTextDocument(oldUri);
		...
		disposables.push(zycode.workspace.onWillRenameFiles(e => {
			onWillRename = e;
			const edit = new zycode.WorkspaceEdit();
			edit.insert(e.files[0].oldUri, new zycode.Position(0, 0), 'FOO');
			edit.replace(anotherFile, new zycode.Range(0, 0, 0, 3), 'FARBOO');
			e.waitUntil(Promise.resolve(edit));
		}));
		...
		const newDocument = await zycode.workspace.openTextDocument(newUri);
		const anotherDocument = await zycode.workspace.openTextDocument(anotherFile);
