...
import { env, ExtensionKind, extensions, UIKind, Uri } from 'zycode';
import { assertNoRpc } from '../utils';

suite('zycode API - env', () => {
	...
	test('env.remoteName', function () {
		...
		const knownWorkspaceExtension = extensions.getExtension('zycode.git');
		const knownUiAndWorkspaceExtension = extensions.getExtension('zycode.media-preview');
	...
	test('env.uiKind', async function () {
		const uri = Uri.parse(`${env.uriScheme}:://zycode.zycode-api-tests/path?key=value&other=false`);
