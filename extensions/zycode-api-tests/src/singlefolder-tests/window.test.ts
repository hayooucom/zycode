...
import { CancellationTokenSource, commands, MarkdownString, TabInputNotebook, Position, QuickPickItem, Selection, StatusBarAlignment, TextEditor, TextEditorSelectionChangeKind, TextEditorViewColumnChangeEvent, TabInputText, Uri, ViewColumn, window, workspace, TabInputTextDiff, UIKind, env } from 'zycode';
import { assertNoRpc, closeAllEditors, createRandomFile, pathEquals } from '../utils';


suite('zycode API - window', () => {
	...
	test('issue #27408 - showTextDocument & zycode.diff always default to ViewColumn.One', async () => {
		const [docA, docB, docC] = await Promise.all([
			workspace.openTextDocument(await createRandomFile()),
			workspace.openTextDocument(await createRandomFile()),
			workspace.openTextDocument(await createRandomFile())
	...
	test('Tabs - zycode.open & zycode.diff', async function () {
		...
		await commands.executeCommand('zycode.open', commandFile, ViewColumn.Three);
		// Ensure active tab is correct after calling zycode.opn
		...
		await commands.executeCommand('zycode.diff', leftDiff, rightDiff, 'Diff', { viewColumn: ViewColumn.Four, preview: false });
	...
	(env.uiKind === UIKind.Web ? test.skip : test)('Tabs - Ensure tabs getter is correct', async function () {
		...
		const leftDiff = await createRandomFile();
		const rightDiff = await createRandomFile();
		await commands.executeCommand('zycode.diff', leftDiff, rightDiff, 'Diff', { viewColumn: ViewColumn.Three, preview: false });
	...
	// TODO@lramos15 https://github.com/microsoft/zycode/issues/145846
	...
	*/
	...
	test('showQuickPick, keep selection (microsoft/zycode-azure-account#67)', async function () {
	...
	test('createStatusBar', async function () {
		...
		assert.strictEqual(statusBarEntryWithoutId.id, 'zycode.zycode-api-tests');
