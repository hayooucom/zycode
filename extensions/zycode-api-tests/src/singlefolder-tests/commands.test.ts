...
import { commands, Position, Range, Uri, ViewColumn, window, workspace } from 'zycode';
import { assertNoRpc, closeAllEditors } from '../utils';

suite('zycode API - commands', () => {
	...
	test('api-command: zycode.diff', function () {
		...
		const a = commands.executeCommand('zycode.diff', Uri.parse('sc:a'), Uri.parse('sc:b'), 'DIFF').then(value => {
			assert.ok(value === undefined);
			registration.dispose();
		});

		const b = commands.executeCommand('zycode.diff', Uri.parse('sc:a'), Uri.parse('sc:b')).then(value => {
			assert.ok(value === undefined);
			registration.dispose();
		});

		const c = commands.executeCommand('zycode.diff', Uri.parse('sc:a'), Uri.parse('sc:b'), 'Title', { selection: new Range(new Position(1, 1), new Position(1, 2)) }).then(value => {
			assert.ok(value === undefined);
			registration.dispose();
		});

		const d = commands.executeCommand('zycode.diff').then(() => assert.ok(false), () => assert.ok(true));
		const e = commands.executeCommand('zycode.diff', 1, 2, 3).then(() => assert.ok(false), () => assert.ok(true));

		return Promise.all([a, b, c, d, e]);
	});

	test('api-command: zycode.open', async function () {
		...
		await commands.executeCommand('zycode.open', uri, ViewColumn.Two);
		...
		await commands.executeCommand('zycode.open', uri, ViewColumn.One);
		assert.strictEqual(window.tabGroups.all.length, 2);
		assert.strictEqual(window.tabGroups.all[0].activeTab?.group.viewColumn, ViewColumn.One);

		let e1: Error | undefined = undefined;
		try {
			await commands.executeCommand('zycode.open');
		} catch (error) {
			e1 = error;
		}
		assert.ok(e1);
		...
		try {
			await commands.executeCommand('zycode.open', uri, true);
		} catch (error) {
			e2 = error;
		}
		assert.ok(e2);
		...
		try {
			await commands.executeCommand('zycode.open', 'file:///some/path/not/http');
	...
	test('api-command: zycode.open with untitled supports associated resource (#138925)', async function () {
		const uri = Uri.parse(workspace.workspaceFolders![0].uri.toString() + '/untitled-file.txt').with({ scheme: 'untitled' });
		await commands.executeCommand('zycode.open', uri).then(() => assert.ok(true), () => assert.ok(false));
