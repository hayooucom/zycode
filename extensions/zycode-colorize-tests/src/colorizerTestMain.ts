...
export function activate(context: zycode.ExtensionContext): any {
	...
	const legend = new zycode.SemanticTokensLegend(tokenTypes, tokenModifiers);

	const outputChannel = zycode.window.createOutputChannel('Semantic Tokens Test');

	const documentSemanticHighlightProvider: zycode.DocumentSemanticTokensProvider = {
		provideDocumentSemanticTokens(document: zycode.TextDocument): zycode.ProviderResult<zycode.SemanticTokens> {
	...
	context.subscriptions.push(zycode.languages.registerDocumentSemanticTokensProvider({ pattern: '**/*semantic-test.json' }, documentSemanticHighlightProvider, legend));
