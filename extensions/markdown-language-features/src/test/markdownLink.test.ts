...
suite('createEditAddingLinksForUriList', () => {
	...
	test('Markdown Link Pasting should occur for a valid link (end to end)', async () => {
		...
		const skinnyDocument: SkinnyTextDocument = {
			uri: zycode.Uri.parse('file:///path/to/your/file'),
			offsetAt: function () { return 0; },
			getText: function () { return 'hello world!'; },
		};

		const result = await createEditAddingLinksForUriList(skinnyDocument, [new zycode.Range(0, 0, 0, 12)], 'https://www.microsoft.com/', true, true, new zycode.CancellationTokenSource().token);
	...
	suite('appendToLinkSnippet', () => {
		...
		test('Should create snippet with < > when pasted link has an mismatched parentheses', () => {
			...
			const snippet = appendToLinkSnippet(new zycode.SnippetString(''), 'abc', uriString, 0, true);
		...
		test('Should create Markdown link snippet when pasteAsMarkdownLink is true', () => {
			...
			const snippet = appendToLinkSnippet(new zycode.SnippetString(''), '', uriString, 0, true);
	...
	suite('checkSmartPaste', () => {
		...
		const skinnyDocument: SkinnyTextDocument = {
			uri: zycode.Uri.file('/path/to/your/file'),
			offsetAt: function () { return 0; },
			getText: function () { return 'hello world!'; },
		};

		test('Should evaluate pasteAsMarkdownLink as true for selected plain text', () => {
			const pasteAsMarkdownLink = checkSmartPaste(skinnyDocument, new zycode.Range(0, 0, 0, 12), new zycode.Range(0, 0, 0, 12));
		...
		test('Should evaluate pasteAsMarkdownLink as false for a valid selected link', () => {
			...
			const pasteAsMarkdownLink = checkSmartPaste(skinnyDocument, new zycode.Range(0, 0, 0, 25), new zycode.Range(0, 0, 0, 25));
		...
		test('Should evaluate pasteAsMarkdownLink as false for a valid selected link with trailing whitespace', () => {
			skinnyDocument.getText = function () { return '   https://www.microsoft.com  '; };
			const pasteAsMarkdownLink = checkSmartPaste(skinnyDocument, new zycode.Range(0, 0, 0, 30), new zycode.Range(0, 0, 0, 30));
			assert.strictEqual(pasteAsMarkdownLink, false);
		});
		...
		test('Should evaluate pasteAsMarkdownLink as true for a link pasted in square brackets', () => {
			...
			const pasteAsMarkdownLink = checkSmartPaste(skinnyDocument, new zycode.Range(0, 1, 0, 4), new zycode.Range(0, 1, 0, 4));
			assert.strictEqual(pasteAsMarkdownLink, true);
		});

		test('Should evaluate pasteAsMarkdownLink as false for no selection', () => {
			const pasteAsMarkdownLink = checkSmartPaste(skinnyDocument, new zycode.Range(0, 0, 0, 0), new zycode.Range(0, 0, 0, 0));
			assert.strictEqual(pasteAsMarkdownLink, false);
		});
		...
		test('Should evaluate pasteAsMarkdownLink as false for selected whitespace and new lines', () => {
			...
			const pasteAsMarkdownLink = checkSmartPaste(skinnyDocument, new zycode.Range(0, 0, 0, 7), new zycode.Range(0, 0, 0, 7));
			assert.strictEqual(pasteAsMarkdownLink, false);
		});

		test('Should evaluate pasteAsMarkdownLink as false for pasting within a backtick code block', () => {
			skinnyDocument.getText = function () { return '```\r\n\r\n```'; };
			const pasteAsMarkdownLink = checkSmartPaste(skinnyDocument, new zycode.Range(0, 5, 0, 5), new zycode.Range(0, 5, 0, 5));
			assert.strictEqual(pasteAsMarkdownLink, false);
		});
		...
		test('Should evaluate pasteAsMarkdownLink as false for pasting within a Markdown link', () => {
			const pasteAsMarkdownLink = checkSmartPaste(linkSkinnyDoc, new zycode.Range(0, 4, 0, 6), new zycode.Range(0, 4, 0, 6));
			assert.strictEqual(pasteAsMarkdownLink, false);
		});
		...
		test('Should evaluate pasteAsMarkdownLink as false for pasting within a Markdown image link', () => {
			const pasteAsMarkdownLink = checkSmartPaste(imageLinkSkinnyDoc, new zycode.Range(0, 5, 0, 10), new zycode.Range(0, 5, 0, 10));
			assert.strictEqual(pasteAsMarkdownLink, false);
		});
		...
		test('Should evaluate pasteAsMarkdownLink as false for pasting within inline code', () => {
			const pasteAsMarkdownLink = checkSmartPaste(inlineCodeSkinnyCode, new zycode.Range(0, 1, 0, 1), new zycode.Range(0, 1, 0, 1));
			assert.strictEqual(pasteAsMarkdownLink, false);
		});
		...
		test('Should evaluate pasteAsMarkdownLink as false for pasting within inline math', () => {
			const pasteAsMarkdownLink = checkSmartPaste(inlineMathSkinnyDoc, new zycode.Range(0, 1, 0, 1), new zycode.Range(0, 1, 0, 1));
			assert.strictEqual(pasteAsMarkdownLink, false);
		});
	});
});
