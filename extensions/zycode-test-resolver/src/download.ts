	...
	return new Promise((resolve, reject) => {
		...
		https.get(requestOptions, res => {
			...
			if (archiveUrl.endsWith('.zip')) {
				const archivePath = path.resolve(destDir, `zycode-server-${commit}.zip`);
				...
				const zipPath = path.resolve(destDir, `zycode-server-${commit}.tgz`);
...
function unzipVSCodeServer(vscodeArchivePath: string, extractDir: string, destDir: string, log: (messsage: string) => void) {
	...
	if (vscodeArchivePath.endsWith('.zip')) {
		const tempDir = fs.mkdtempSync(path.join(destDir, 'zycode-server-extract'));
		...
		fs.renameSync(path.join(tempDir, process.platform === 'win32' ? 'zycode-server-win32-x64' : 'zycode-server-darwin-x64'), extractDir);
	} else {
