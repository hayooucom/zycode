...
export function activate(_context: zycode.ExtensionContext) {
	zycode.workspace.registerRemoteAuthorityResolver('test', {
		async resolve(_authority: string): Promise<zycode.ResolverResult> {
			console.log(`Resolving ${_authority}`);
			console.log(`Activating zycode.github-authentication to simulate auth`);
			await zycode.extensions.getExtension('zycode.github-authentication')?.activate();
			return new zycode.ManagedResolvedAuthority(async () => {
 ...
 * actual WebSocket.
 */
class InitialManagedMessagePassing implements zycode.ManagedMessagePassing {
	private readonly dataEmitter = new zycode.EventEmitter<Uint8Array>();
	private readonly closeEmitter = new zycode.EventEmitter<Error | undefined>();
	private readonly endEmitter = new zycode.EventEmitter<void>();
...
class OpeningManagedMessagePassing {
	...
	constructor(
		url: URL,
		dataEmitter: zycode.EventEmitter<Uint8Array>,
		closeEmitter: zycode.EventEmitter<Error | undefined>,
		_endEmitter: zycode.EventEmitter<void>
	) {
