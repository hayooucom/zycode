...
export class SettingsDocument {
	...
	public async provideCompletionItems(position: zycode.Position, _token: zycode.CancellationToken): Promise<zycode.CompletionItem[] | zycode.CompletionList> {
	...
	private isCompletingPropertyValue(location: Location, pos: zycode.Position) {
	...
	private async provideWindowTitleCompletionItems(location: Location, pos: zycode.Position): Promise<zycode.CompletionItem[]> {
		const completions: zycode.CompletionItem[] = [];
		...
		if (!range || range.start.isEqual(pos) || range.end.isEqual(pos) && this.document.getText(range).endsWith('}')) {
			range = new zycode.Range(pos, pos);
		}
		...
		completions.push(this.newSimpleCompletionItem(getText('activeEditorShort'), range, zycode.l10n.t("the file name (e.g. myFile.txt)")));
		completions.push(this.newSimpleCompletionItem(getText('activeEditorMedium'), range, zycode.l10n.t("the path of the file relative to the workspace folder (e.g. myFolder/myFileFolder/myFile.txt)")));
		completions.push(this.newSimpleCompletionItem(getText('activeEditorLong'), range, zycode.l10n.t("the full path of the file (e.g. /Users/Development/myFolder/myFileFolder/myFile.txt)")));
		completions.push(this.newSimpleCompletionItem(getText('activeFolderShort'), range, zycode.l10n.t("the name of the folder the file is contained in (e.g. myFileFolder)")));
		completions.push(this.newSimpleCompletionItem(getText('activeFolderMedium'), range, zycode.l10n.t("the path of the folder the file is contained in, relative to the workspace folder (e.g. myFolder/myFileFolder)")));
		completions.push(this.newSimpleCompletionItem(getText('activeFolderLong'), range, zycode.l10n.t("the full path of the folder the file is contained in (e.g. /Users/Development/myFolder/myFileFolder)")));
		completions.push(this.newSimpleCompletionItem(getText('rootName'), range, zycode.l10n.t("name of the workspace with optional remote name and workspace indicator if applicable (e.g. myFolder, myRemoteFolder [SSH] or myWorkspace (Workspace))")));
		completions.push(this.newSimpleCompletionItem(getText('rootNameShort'), range, zycode.l10n.t("shortened name of the workspace without suffixes (e.g. myFolder or myWorkspace)")));
		completions.push(this.newSimpleCompletionItem(getText('rootPath'), range, zycode.l10n.t("file path of the workspace (e.g. /Users/Development/myWorkspace)")));
		completions.push(this.newSimpleCompletionItem(getText('folderName'), range, zycode.l10n.t("name of the workspace folder the file is contained in (e.g. myFolder)")));
		completions.push(this.newSimpleCompletionItem(getText('folderPath'), range, zycode.l10n.t("file path of the workspace folder the file is contained in (e.g. /Users/Development/myFolder)")));
		completions.push(this.newSimpleCompletionItem(getText('appName'), range, zycode.l10n.t("e.g. VS Code")));
		completions.push(this.newSimpleCompletionItem(getText('remoteName'), range, zycode.l10n.t("e.g. SSH")));
		completions.push(this.newSimpleCompletionItem(getText('dirty'), range, zycode.l10n.t("an indicator for when the active editor has unsaved changes")));
		completions.push(this.newSimpleCompletionItem(getText('separator'), range, zycode.l10n.t("a conditional separator (' - ') that only shows when surrounded by variables with values")));

		return completions;
	}

	private async provideFilesAssociationsCompletionItems(location: Location, position: zycode.Position): Promise<zycode.CompletionItem[]> {
		const completions: zycode.CompletionItem[] = [];
		...
		if (location.path.length === 2) {
			...
			if (location.path[1] === '') {
				...
				completions.push(this.newSnippetCompletionItem({
					label: zycode.l10n.t("Files with Extension"),
					documentation: zycode.l10n.t("Map all files matching the glob pattern in their filename to the language with the given identifier."),
					snippet: location.isAtPropertyKey ? '"*.${1:extension}": "${2:language}"' : '{ "*.${1:extension}": "${2:language}" }',
					range
				}));

				completions.push(this.newSnippetCompletionItem({
					label: zycode.l10n.t("Files with Path"),
					documentation: zycode.l10n.t("Map all files matching the absolute path glob pattern in their path to the language with the given identifier."),
	...
	private async provideExcludeCompletionItems(location: Location, position: zycode.Position): Promise<zycode.CompletionItem[]> {
		const completions: zycode.CompletionItem[] = [];
		...
		if (location.path.length === 1 || (location.path.length === 2 && location.path[1] === '')) {
			...
			completions.push(this.newSnippetCompletionItem({
				label: zycode.l10n.t("Files by Extension"),
				documentation: zycode.l10n.t("Match all files of a specific file extension."),
				snippet: location.path.length === 2 ? '"**/*.${1:extension}": true' : '{ "**/*.${1:extension}": true }',
				range
			}));

			completions.push(this.newSnippetCompletionItem({
				label: zycode.l10n.t("Files with Multiple Extensions"),
				documentation: zycode.l10n.t("Match all files with any of the file extensions."),
				snippet: location.path.length === 2 ? '"**/*.{ext1,ext2,ext3}": true' : '{ "**/*.{ext1,ext2,ext3}": true }',
				range
			}));

			completions.push(this.newSnippetCompletionItem({
				label: zycode.l10n.t("Files with Siblings by Name"),
				documentation: zycode.l10n.t("Match files that have siblings with the same name but a different extension."),
			...
			completions.push(this.newSnippetCompletionItem({
				label: zycode.l10n.t("Folder by Name (Top Level)"),
				documentation: zycode.l10n.t("Match a top level folder with a specific name."),
				snippet: location.path.length === 2 ? '"${1:name}": true' : '{ "${1:name}": true }',
				range
			}));

			completions.push(this.newSnippetCompletionItem({
				label: zycode.l10n.t("Folders with Multiple Names (Top Level)"),
				documentation: zycode.l10n.t("Match multiple top level folders."),
				snippet: location.path.length === 2 ? '"{folder1,folder2,folder3}": true' : '{ "{folder1,folder2,folder3}": true }',
				range
			}));

			completions.push(this.newSnippetCompletionItem({
				label: zycode.l10n.t("Folder by Name (Any Location)"),
				documentation: zycode.l10n.t("Match a folder with a specific name in any location."),
				snippet: location.path.length === 2 ? '"**/${1:name}": true' : '{ "**/${1:name}": true }',
				range
			}));
		}
	...
	private async provideLanguageCompletionItems(location: Location, position: zycode.Position): Promise<zycode.CompletionItem[]> {
		if (location.path.length === 1 && this.isCompletingPropertyValue(location, position)) {
			const range = this.getReplaceRange(location, position);
			const languages = await zycode.languages.getLanguages();
			return [
				this.newSimpleCompletionItem(JSON.stringify('${activeEditorLanguage}'), range, zycode.l10n.t("Use the language of the currently active text editor if any")),
	...
	private async provideLanguageCompletionItemsForLanguageOverrides(range: zycode.Range): Promise<zycode.CompletionItem[]> {
		const languages = await zycode.languages.getLanguages();
		const completionItems = [];
		for (const language of languages) {
			const item = new zycode.CompletionItem(JSON.stringify(language));
			item.kind = zycode.CompletionItemKind.Property;
			item.range = range;
	...
	private async provideLanguageOverridesCompletionItems(location: Location, position: zycode.Position): Promise<zycode.CompletionItem[]> {
		if (location.path.length === 1 && location.isAtPropertyKey && location.previousNode && typeof location.previousNode.value === 'string' && location.previousNode.value.startsWith('[')) {
			...
			const languageOverridesRanges: zycode.Range[] = [];
			let matches = OVERRIDE_IDENTIFIER_REGEX.exec(location.previousNode.value);
			let lastLanguageOverrideRange: zycode.Range | undefined;
			while (matches?.length) {
				lastLanguageOverrideRange = new zycode.Range(this.document.positionAt(location.previousNode.offset + 1 + matches.index), this.document.positionAt(location.previousNode.offset + 1 + matches.index + matches[0].length));
			...
			if (lastLanguageOverrideEndPosition.isBefore(endPosition)) {
				languageOverridesRanges.push(new zycode.Range(lastLanguageOverrideEndPosition, endPosition));
			}
			...
			if (languageOverrideRange && !languageOverrideRange.isEqual(languageOverridesRanges[0])) {
				...
				for (const language of languages) {
					if (!donotSuggestLanguages.includes(language)) {
						const item = new zycode.CompletionItem(`[${language}]`);
						item.kind = zycode.CompletionItemKind.Property;
	...
	private providePortsAttributesCompletionItem(range: zycode.Range): zycode.CompletionItem[] {
